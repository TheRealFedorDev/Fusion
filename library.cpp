...
Tensor Tensor::sum(int axis) const {
    if (shape.size() != 2) {
        throw std::invalid_argument("sum with axis currently supports only 2D tensors");
    }
    int m = shape[0];
    int n = shape[1];
    ...
    if (axis == 0) {
        ...
        return Tensor(result, {m});
    }
    else {
        throw std::invalid_argument("Invalid axis for 2D tensor");
    }
}
...
Tensor Tensor::mean(int axis) const {
    return this->sum(axis).divide(static_cast<float>(shape[axis]));
}
...
Tensor Tensor::divide(const Tensor& other) const {
    if (data.size() != other.data.size()) {
        throw std::invalid_argument("Tensor size mismatch in divide");
    }
    std::vector<float> result(data.size());
    for (size_t i = 0; i < data.size(); ++i) {
        if (other.data[i] == 0.0f) throw std::invalid_argument("Division by zero");
        result[i] = data[i] / other.data[i];
    }
    return Tensor(result, shape);
}
...
PYBIND11_MODULE(_fusion_core, m) {
    ...
    py::class_<Tensor>(m, "Tensor")
        ...
        .def("sum", [](const Tensor& self, int axis = -1) -> py::object {
        if (axis == -1) {
            return py::float_(self.sum());          //  float
        } else {
            return py::cast(self.sum(axis));        //  Tensor
        }
        }, py::arg("axis") = -1, "Sum all elements or along an axis")
        ...
        .def("__truediv__", [](const Tensor& self, const py::object& other) -> Tensor {
        if (py::isinstance<py::float_>(other) || py::isinstance<py::int_>(other)) {
            return self.divide(other.cast<float>());
        } else if (py::isinstance<Tensor>(other)) {
            return self.divide(other.cast<Tensor>());
        } else {
            throw std::invalid_argument("Unsupported operand type for /");
        }
        }, "Divide by scalar or another tensor")
        .def("matmul", &Tensor::matmul, "Matrix multiplication")
        .def("transpose", &Tensor::transpose, "Transpose 2D tensor").def("mean", [](const Tensor& self, int axis = -1) -> py::object {
        if (axis == -1) {
            return py::float_(self.mean());          // общая средняя
        } else {
            return py::cast(self.mean(axis));        // среднее по оси
        }
        }, py::arg("axis") = -1, "Mean of all elements or along an axis")
        .def("__pow__", [](const Tensor& self, const py::object& other) -> Tensor {
        if (py::isinstance<py::float_>(other) || py::isinstance<py::int_>(other)) {
            return self.pow(other.cast<float>());
        } else if (py::isinstance<Tensor>(other)) {
            // поэлементное возведение в степень тензора – пока не нужно, но можно вернуть ошибку (пока лень)
            throw std::invalid_argument("Tensor ** Tensor not supported yet");
        } else {
            throw std::invalid_argument("Unsupported operand type for **");
        }
        }, "Raise each element to a power (scalar)")
